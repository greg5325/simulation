...
vector<unsigned int> selfdualsimulation(vector<char> & labels, vector<vector<int> > & outEdges) {
...
//	vector<unsigned int> removes_suc;
	...
	while (exist) {
		...
		if (removes_pre[v]) {
					...
					for (unsigned int w = 0; w < labels.size(); w++) {
						if (removes_pre[v] & 1 << w) { //for all w in remove(v)
							if (sims[u] & 1 << w) { //if w in sim(u)
								...
								for (unsigned int w__index = 0; w__index < outEdges[w].size();
										w__index++) {
			...
			for (unsigned int u_index = 0; u_index < outEdges[v].size(); u_index++) { //for all u in suc(v)
